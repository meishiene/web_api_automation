<template>
  <section class="batches-page">
    <div class="hero-card">
      <div>
        <router-link :to="`/project/${projectId}/web-test-cases`" class="back-link">← 返回 UI 自动化</router-link>
        <span class="eyebrow">Web Batch Runs</span>
        <h2>Web 批次结果</h2>
        <p>查看当前项目下 UI 自动化批量执行历史，快速定位失败批次。</p>
      </div>
    </div>
    ...
    <section class="panel-card">
      ...
      <div v-else-if="batches.length === 0" class="state-block">
        <p>暂无 Web 批量执行记录，先在 UI 自动化页触发批量执行。</p>
      </div>
        ...
        <table class="batches-table">
          ...
          <tbody>
            <tr v-for="batch in batches" :key="batch.id">
              ...
              <td>
                <router-link class="table-link" :to="`/project/${projectId}/web-batches/${batch.id}`">查看详情</router-link>
              </td>
            </tr>
          </tbody>
        </table>
...
import { getWebBatchRuns } from '@/api/webTestRuns'
...
const fetchBatches = async () => {
  loading.value = true
  try {
    batches.value = await getWebBatchRuns(projectId)
  } catch {
    alert('获取 Web 批次列表失败')
  } finally {
    loading.value = false
