...
| 调度与队列 | 阶段 4 已完成 S4-01~S4-04：调度/队列/Worker 最小闭环 + 最小可视化已落地；S4-05 验收中（后端全量回归与迁移链路已通过；前端构建门禁需在可运行构建的环境/CI复核；含 R1 主循环最小实现） | 42% |
| 报告与分析 | 仅有基础结果记录 | 5% |
...
| 阶段 4 | 调度与分布式执行 | 启动中 | 已完成 S4-01~S4-04（编排骨架、调度最小 API、队列与 Worker 最小闭环、最小可视化）；S4-05 验收中（后端全量回归与迁移链路已通过；前端构建门禁需在可运行构建的环境/CI复核）；R2~R5 转后续迭代 |
...
- 已落地 Worker 运行时服务与循环脚本：`app/services/queue_worker_runtime.py`、`scripts/run_queue_worker_loop.py`（真实消费主循环最小实现）
...
- 已落地最小 Worker 闭环：任务领取、真实执行一次回写、状态回写与心跳上报
...
- 已完成 S4-03：队列与 Worker 最小闭环（run_queue + Worker 心跳/真实执行一次）
- 已完成 S4-04：执行管理最小可视化（Scheduling Dashboard）
- S4-05 验收中：阶段验收与切换准备（后端全量回归与迁移链路已通过；前端构建门禁需在可运行构建的环境/CI复核；R1 主循环最小实现已落地）
...
- 已完成 S4-05：阶段验收与切换准备（R1 主循环最小实现已落地）
...
- 阶段 4 推进：完成 S4-03（队列与 Worker 最小闭环），新增 `run_queue` 领取/回写接口、Worker 执行接口与心跳接口（`/api/run-queue/*`）
...
- S4-05 execution run completed: backend full regression passed (95 passed), frontend build passed, and migration chain check passed after legacy SQLite revision repair.

- Migration risk RISK-S4-001 closed: added `scripts/legacy_sqlite_revision_repair.py` and repaired `test_platform.db` revision drift (`01fcc228897e -> e2b4c6a8d901`), then revalidated migration chain.
